...
public class TrainControllerManager {
  @FXML public CheckBox HWTrainCheckBox;
  ...
  @FXML public TextArea statusLog;
  @FXML private Text nextStationText;
  @FXML
  private CheckBox intLightCheckBox,
      extLightCheckBox,
      openDoorLeftCheckBox,
      openDoorRightCheckBox,
      toggleServiceBrakeCheckBox,
      autoModeCheckBox;
  @FXML
  private TextField setTemperatureTextField, setKiTextField, setKpTextField, setSpeedTextField;
  @FXML private Button emergencyBrakeButton, makeAnnouncementsButton;
  @FXML private Slider setSpeedSlider;
  @FXML
  private Gauge currentSpeedGauge,
      speedLimitGauge,
      commandedSpeedGauge,
      authorityGauge,
      powerOutputGauge,
  ...
  @FXML
  private Circle eBrakeStatus,
      signalFailureStatus,
      brakeFailureStatus,
      powerFailureStatus,
      stationSideLeftStatus,
      stationSideRightStatus,
      inTunnelStatus;
  @FXML private ChoiceBox<Integer> trainNoChoiceBox;
  ...
  private final ScheduledExecutorService textUpdateExecutor =
  ...
  private void setupMapChangeListener() {
    ...
    // Defining a generic listener for the map, overriding the methods to only update the choice box
    // items
    ObservableHashMap.MapListener<Integer, TrainControllerSubject> genericListener =
        new ObservableHashMap.MapListener<>() {
          ...
          public void onUpdated(
              Integer key, TrainControllerSubject oldValue, TrainControllerSubject newValue) {
            updateChoiceBoxItems();
          }
        };
  ...
  private void bindGauges() {
    appendListener(
        currentSubject.getDoubleProperty(CURRENT_SPEED),
        (obs, oldVal, newVal) -> {
          if (Math.abs(currentSpeedGauge.getValue() - newVal.doubleValue()) < 0.01) {
            return;
          } // Only update if there is a significant change (0.1 difference)
          currentSpeedGauge.setValue(newVal.doubleValue());
          //        logger.debug("Current speed gauge updated to {}", newVal);
        });
    appendListener(
        currentSubject.getDoubleProperty(COMMAND_SPEED),
        (obs, oldVal, newVal) -> {
    ...
    appendListener(
        currentSubject.getDoubleProperty(SPEED_LIMIT),
        (obs, oldVal, newVal) -> {
          if (Math.abs(oldVal.doubleValue() - newVal.doubleValue()) < 0.01) {
            return;
          } // Only update if there is a significant change (0.1 difference)
    ...
    appendListener(
        currentSubject.getIntegerProperty(AUTHORITY),
        (obs, oldVal, newVal) -> {
          authorityGauge.setValue(newVal.intValue());
          logger.debug("Authority gauge updated to {}", newVal);
        });
    appendListener(
        currentSubject.getDoubleProperty(SET_TEMPERATURE),
        (obs, oldVal, newVal) -> {
          if (Math.abs(currentTemperatureGauge.getValue() - newVal.doubleValue()) < 0.1) {
            return;
          } // Only update if there is a significant change (0.1 difference)
    ...
    appendListener(
        currentSubject.getDoubleProperty(POWER),
        (obs, oldVal, newVal) -> {
          if (Math.abs(powerOutputGauge.getValue() - newVal.doubleValue()) < 0.01) {
            return;
          } // Only update if there is a significant change (0.1 difference)
  ...
  private void bindIndicators() {
    appendListener(
        currentSubject.getBooleanProperty(EMERGENCY_BRAKE),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(SIGNAL_FAILURE),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(BRAKE_FAILURE),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(POWER_FAILURE),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(IN_TUNNEL),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(LEFT_PLATFORM),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
    ...
    appendListener(
        currentSubject.getBooleanProperty(RIGHT_PLATFORM),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
  ...
  private void bindStringText(Text text) {
    appendListener(
        currentSubject.getProperty(NEXT_STATION),
        (obs, oldVal, newVal) -> Platform.runLater(() -> text.setText((String) newVal)));
    appendListener(
        currentSubject.getProperty(ARRIVAL_STATION),
        (obs, oldVal, newVal) ->
            Platform.runLater(
                () -> {
                  if (newVal != null) {
  ...
  private void bindControls() {
    ...
    appendListener(
        currentSubject.getStringProperty(ERROR),
        (obs, oldVal, newVal) -> {
  ...
  private void setupButtonActions() {
    emergencyBrakeButton.setOnAction(
        event -> {
    ...
    makeAnnouncementsButton.setOnAction(
        event -> {
          ...
          if (!nextStationText.getText().contains("yard")
              && !nextStationText.getText().contains("Yard")) {
            sendAlert("Arriving at " + nextStationText.getText());
          }
        });

    killTrain.setOnAction(
        event -> {
          if (TrainControllerFactory.getTrainLock()) {
          ...
          logger.warn(
              "Killing Train Controller for train ID: {}", currentSubject.getController().getID());
  ...
  private void bindSliderAndTextField(
      Slider slider, TextField textField, Consumer<Double> consumer) {
    appendListener(
        slider.valueProperty(),
        (obs, oldVal, newVal) -> {
          if (Math.abs(oldVal.doubleValue() - newVal.doubleValue()) < 0.1) {
            return;
          }
  ...
  private void unbindControls() {
    ...
    setSpeedSlider
        .valueProperty()
        .unbind(); // Assuming you might bind this bidirectionally in another part of your code
  ...
  private void updateView() {
    ...
    // Update indicators
    updateIndicator(
        Color.RED, eBrakeStatus, currentSubject.getBooleanProperty(EMERGENCY_BRAKE).get());
    updateIndicator(
        Color.RED, signalFailureStatus, currentSubject.getBooleanProperty(SIGNAL_FAILURE).get());
    updateIndicator(
        Color.RED, brakeFailureStatus, currentSubject.getBooleanProperty(BRAKE_FAILURE).get());
    updateIndicator(
        Color.RED, powerFailureStatus, currentSubject.getBooleanProperty(POWER_FAILURE).get());
    updateIndicator(
        Color.YELLOW, inTunnelStatus, currentSubject.getBooleanProperty(IN_TUNNEL).get());
    updateIndicator(
        Color.LIGHTGREEN,
        stationSideLeftStatus,
        currentSubject.getBooleanProperty(LEFT_PLATFORM).get());
    updateIndicator(
        Color.LIGHTGREEN,
        stationSideRightStatus,
        currentSubject.getBooleanProperty(RIGHT_PLATFORM).get());
    ...
    // Update text fields
    setTemperatureTextField.setText(
        String.format("%.2f", currentSubject.getDoubleProperty(SET_TEMPERATURE).get()));
    ...
    setSpeedTextField.setText(
        String.format("%.2f", currentSubject.getDoubleProperty(OVERRIDE_SPEED).get()));
  ...
  private void inTunnelUpdates() {
    ...
    if (autoModeCheckBox.isSelected()) { // Runs in Auto Mode
      ...
      String colorFormat =
          inTunnel ? "-fx-background-color: #000033;" : "-fx-background-color: #FFFFFF;";
  ...
  private void setNotification(ControllerProperty propertyName, String value) {
        ...
        switch (propertyName) {
          ...
          case SERVICE_BRAKE,
              ...
              + (Boolean.parseBoolean(value)
                  ? getOnLabel(propertyName)
                  : getOffLabel(propertyName));
          case EMERGENCY_BRAKE -> "\nEmergency Brake \n"
              + (Boolean.parseBoolean(value)
                  ? getOffLabel(propertyName)
                  : getOnLabel(
                      propertyName)); // For some reason emergency brake boolean values are flipped
    ...
    Platform.runLater(
        () -> {
          ...
          //         logger.info("Status notification set for {}: {}", propertyName,
          // statusNotification.trim());
        });
  }
    ...
    synchronized (lock) {
      if (scheduledFuture == null || scheduledFuture.isDone()) {
        if (!textUpdateQueue.isEmpty()) {
          ...
          scheduledFuture =
              textUpdateExecutor.schedule(task::run, task.delay, TimeUnit.MILLISECONDS);
        }
      }
    }
  }
