...
// TODO: The Subject is now entirely storing imperial units, and here we have only metric units. We
// convert them all here, and it is consistent.

/**
 * Profetta Notes: The train controller is meant to stop if a command speed is not sent. The wayside
 * is not able to send a corrected speed, rather it just chooses not to send a speed at all, and if
 * the train does not receive a speed, it should stop.
 */

/**
 * This method is used to calculate the power needed for the train. It first determines the set
 * speed based on whether the train is in automatic mode or not. Then it calculates the current
 * speed and initializes acceleration to 0. It calculates the error between the set speed and the
 * current speed, and updates the rolling error. The power is then calculated using the
 * proportional-integral (PI) controller formula. If the calculated power exceeds the maximum power,
 * it is set to the maximum power. If the train is in automatic mode and the calculated power is
 * less than 0, the power is set to 0 and the service brake is activated. If the service brake is
 * active and the calculated power is greater than 0, the service brake is deactivated. If the
 * emergency brake or service brake is active, or there is a power failure, or the calculated power
 * is less than 0, the power is set to 0. If the emergency brake is active, the acceleration is set
 * to the emergency brake deceleration. If the service brake is active, the acceleration is set to
 * the service brake deceleration. Otherwise, the acceleration is calculated by dividing the power
 * by the weight of the train. The current speed is then updated based on the calculated
 * acceleration. If the current speed is less than 0, it is set to 0. Finally, the speed and power
 * of the train are updated.
 */
public class TrainControllerImpl implements TrainController {
  ...
  /**
   * This method is used to assign a TrainModel object to the trainControllerImpl. It first assigns
   * the passed TrainModel object to the train variable of the trainControllerImpl. Then it sets the
   * service brake, emergency brake, internal lights, external lights, left doors, right doors,
   * temperature, signal failure, brake failure, and power failure of the trainControllerImpl to the
   * corresponding values of the passed TrainModel object.
   *
  ...
  public void onStation() {
    ...
    if (train.getSpeed() == 0) {
      ...
      if (stopTime.getValue() < (60 * 8)) {
        ...
        logger.info(
            "Train stopped at station {} for {} seconds",
            nextStationName,
            (float) stopTime.getValue() / 8);
        waysideStop = false;
        stopTime.setValue(0);
      }
        ...
        notificationExecutor.execute(
            () ->
                subject.notifyChange(
                    AUTHORITY, convertDistance(authority, distanceUnit.METERS, distanceUnit.FEET)));
        break;
    }
  }
  ...
  public void updateBeacon(Beacon beacon) {
    ...
    if (currentBeacon != null) {
      ...
      // Note: We could deal with this case by just setting the "head" of the beacon equal to the
      // end
  ...
  public void setIntLights(boolean lights) {
    ...
    notificationExecutor.execute(
        () ->
            subject.notifyChange(
                INT_LIGHTS, lights)); // This might've been the issue interiorLights -> intLights
  }
  ...
  public void setExtLights(boolean lights) {
    ...
    notificationExecutor.execute(
        () ->
            subject.notifyChange(
                EXT_LIGHTS, lights)); // This might've been the issue exteriorLights -> extLights
  }
  ...
  public void setCurrentTemperature(double temp) {
    ...
    notificationExecutor.execute(
        () ->
            subject.notifyChange(
                CURRENT_TEMPERATURE, convertTemperature(temp, CELSIUS, FAHRENHEIT)));
  }
  ...
  public void setSpeedLimit(double limit) {
    ...
    notificationExecutor.execute(
        () -> subject.notifyChange(SPEED_LIMIT, convertVelocity(limit, KPH, MPH)));
  }
  ...
  /**
   * This method is used to set the value of a property based on the property name. It uses a switch
   * statement to determine which property to set. The method casts the newValue parameter to the
   * appropriate type based on the property. If the property name is not found, it prints an error
   * message to the console. After setting the property, it does not calculate power.
   *
  ...
  public void setValue(ControllerProperty propertyName, Object newValue) {
    switch (propertyName) {
      ...
      case SET_TEMPERATURE -> this.setTemperature =
          convertTemperature((double) newValue, FAHRENHEIT, CELSIUS);
      case CURRENT_TEMPERATURE -> this.currentTemperature =
          convertTemperature((double) newValue, FAHRENHEIT, CELSIUS);
  ...
  //                        logger.info("Arriving at station: {}",
  // updatedSourceBlock.stationName());
  //                    } else {
  //                        logger.info("Entering switch: {}", updatedSourceBlock.blockNumber());
  //                    }
  //                } else if (currentSourceBlock.blockNumber() ==
  // previousSourceBlock.blockNumber()) {
