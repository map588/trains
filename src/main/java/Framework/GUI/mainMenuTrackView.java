...
//          contextMenu.getItems().add(openInTab);
    ...
    private Node createModuleContent(String moduleName) {
        try {
            String fxmlFile = "/Framework/GUI/FXML/" + moduleName + ".fxml";
            URL url = getClass().getResource(fxmlFile);
            FXMLLoader loader = new FXMLLoader(url);
            Node content = loader.load();

            if (moduleName.equals("trackModel")) {
                // Parse the CSV file and create the track layout
                ConcurrentHashMap<Lines, ConcurrentSkipListMap<Integer, BasicBlock>> trackLayout = BlockParser.parseCSV();

                // Create an instance of TrackVisualizer and add it to the content
                TrackVisualizer trackVisualizer = new TrackVisualizer(trackLayout);
                ((VBox) content).getChildren().add(trackVisualizer);
            }
