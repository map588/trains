...
public class Main {
  ...
  private static final ExecutorService synchronizationPool =
  ...
  private static class TimeSynchronizationTask implements Runnable {
    ...
    public void startScheduling(
        ScheduledExecutorService scheduledExecutorService, long initialTimestep) {
    ...
    private void scheduleTask() {
      Main.scheduledTask =
          scheduledExecutorService.scheduleAtFixedRate(
              this, 0, Main.TIMESTEP, TimeUnit.MILLISECONDS);
    }

    public TimeSynchronizationTask(
        TrackSystem trackSystem,
        WaysideSystem waysideController,
        TrainSystem trainSystem,
        CTCOfficeImpl ctcOffice) {
    ...
    public void run() {
      ...
      if (++timeIndex
          >= updatesPerSimSecond) { // We only need to update the time label once per simulated
        // second
        timeIndex = 0;
        ...
        Platform.runLater(
            () ->
                mainMenu.timeLabel.setText(
                    String.format(
                        "Time: %02d:%02d:%02d", simHour, simMinute, ((int) simSecond) % 60)));
      }
