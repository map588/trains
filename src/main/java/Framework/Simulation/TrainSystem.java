...
public class TrainSystem {
  private static final Logger logger = LoggerFactory.getLogger(TrainSystem.class);

  ExecutorService trainExecutor = Executors.newWorkStealingPool(4);

  private final ConcurrentHashMap<TrainModel, TrainUpdateTask> updateTasks =
  ...
  private class TrainUpdateTask implements Callable<Void> {
    ...
    public Void call() {
      ...
      try {
        Future<UpdatedTrainValues> utvFuture =
            trainExecutor.submit(controller::sendUpdatedTrainValues);
