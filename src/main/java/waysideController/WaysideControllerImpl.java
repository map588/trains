...
public class WaysideControllerImpl implements WaysideController, PLCRunner, Notifier {
  ...
  public WaysideControllerImpl(
    ...
    ConcurrentSkipListMap<Integer, BasicBlock> blockList =
        BasicBlockParser.getInstance().getBasicLine(trackLine);
    ...
    if (!isRunningPLC()) {
      ...
      if (!maintenanceMode) {
        for (int plcIndex = 0; plcIndex < plcPrograms.length; plcIndex++) {
          ...
          //                System.out.println("PLC Results[" + plcIndex + "]: " +
          // plcResults[plcIndex].size() + " " + currentPLCResult.size());
        ...
        for (int changeIndex = 0; changeIndex < changeSize; changeIndex++) {
          ...
          //                System.out.println("PLC Change: " + change.changeType + " " +
          // change.blockID + " " + change.changeValue + " " + plcResults[0].size());
          logger.info(
              "PLC Change: {} {} {}", change.changeType(), change.blockID(), change.changeValue());
      ...
      for (WaysideBlock block : blockMap.values()) {
        if (block.isOccupied()) {
          trackModel.setTrainAuthority(
              block.getBlockID(), block.getBooleanAuth() ? RESUME_TRAIN_SIGNAL : STOP_TRAIN_SIGNAL);
      ...
      while (!occupancyBlockToIDStack.isEmpty()) {
        ...
        if (blockFrom == -1) {
          logger.info(
              "Setting occupancy for block {} to {} from QUEUE",
              occupancyBlockToIDStack.peek(),
              occupancyValStack.peek());
          setOccupancy(occupancyBlockToIDStack.pop(), occupancyValStack.pop());
        } else {
          logger.info(
              "Moving occupancy from block {} to {} from QUEUE",
              blockFrom,
              occupancyBlockToIDStack.peek());
  ...
  public void trackModelSetOccupancy(int blockID, boolean occupied) {
    ...
    if (ctcOffice != null) ctcOffice.setBlockOccupancy(trackLine, blockID, occupied);
  }
  ...
  public void CTCChangeBlockMaintenanceState(int blockID, boolean maintenanceState) {
    ...
    if (currentState != maintenanceState && (!maintenanceState || !block.isOccupied())) {
      ...
      if (ctcOffice != null) ctcOffice.setBlockMaintenance(trackLine, blockID, maintenanceState);
  ...
  public void CTCEnableAllBlocks() {
    ...
    for (WaysideBlock block : blockMap.values()) {
      if (!block.inMaintenance()) {
        ...
        if (ctcOffice != null) ctcOffice.setBlockMaintenance(trackLine, block.getBlockID(), false);
  ...
  public void maintenanceSetSwitch(int blockID, boolean switchState) {
    if (maintenanceMode || blockMap.get(blockID).inMaintenance()) {
      ...
      if (trackModel != null) trackModel.setSwitchState(blockID, switchState);
      if (ctcOffice != null) ctcOffice.setSwitchState(trackLine, blockID, switchState);
    }
  }
  ...
  public void maintenanceSetTrafficLight(int blockID, boolean lightState) {
    ...
    if (maintenanceMode || blockMap.get(blockID).inMaintenance()) {
      ...
      if (trackModel != null) trackModel.setLightState(blockID, lightState);
      if (ctcOffice != null) ctcOffice.setLightState(trackLine, blockID, lightState);
  ...
  public void maintenanceSetCrossing(int blockID, boolean crossingState) {
    ...
    if (maintenanceMode || blockMap.get(blockID).inMaintenance()) {
      ...
      if (trackModel != null) trackModel.setCrossing(blockID, crossingState);
      if (ctcOffice != null) ctcOffice.setCrossingState(trackLine, blockID, crossingState);
  ...
  public void CTCSendAuthority(int blockID, int blockCount) {
    ...
    if (blockMap.get(blockID).isOccupied()) {
      if (trackModel != null) trackModel.setTrainAuthority(blockID, blockCount);
      else logger.error("Track Model is null");
    } else {
  ...
  /**
   * This method is used to notify the change in the value of a property. It prints the property
   * name and the new value to the console and then notifies the subject of the change. The
   * notification to the subject is only done if the GUI is not currently being updated.
   *
