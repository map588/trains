...
public class WaysideControllerHWBridge implements WaysideController, Notifier {
  ...
  public WaysideControllerHWBridge(
      int id,
      Lines trackLine,
      int[] blockIDList,
      int[] outsideOccupancyBlockList,
      String comPort,
    ...
    ConcurrentSkipListMap<Integer, BasicBlock> blockList =
        BasicBlockParser.getInstance().getBasicLine(trackLine).toConcurrentSkipListMap();
  ...
  public void CTCSendSpeed(int blockID, double speed) {
    ...
    if (blockMap.get(blockID).isOccupied()) {
      if (trackModel != null) trackModel.setCommandedSpeed(blockID, speed);
    } else {
  ...
  private void parseCOMMessage(String message) {
    ...
    switch (values[0]) {
      case "switchState" -> {
        ...
        if (trackModel != null) trackModel.setSwitchState(blockID, boolVal);
        if (ctcOffice != null) ctcOffice.setSwitchState(trackLine, blockID, boolVal);
      }
      case "trafficLight" -> {
        boolean boolVal = Boolean.parseBoolean(setValues[1]);
        blockMap.get(blockID).setLightState(boolVal);
        if (trackModel != null) trackModel.setLightState(blockID, boolVal);
        if (ctcOffice != null) ctcOffice.setLightState(trackLine, blockID, boolVal);
      }
      case "crossing" -> {
        boolean boolVal = Boolean.parseBoolean(setValues[1]);
        blockMap.get(blockID).setCrossingState(boolVal);
        if (trackModel != null) trackModel.setCrossing(blockID, boolVal);
        if (ctcOffice != null) ctcOffice.setCrossingState(trackLine, blockID, boolVal);
      }
      case "auth" -> {
        boolean boolVal = Boolean.parseBoolean(setValues[1]);
        WaysideBlock block = blockMap.get(blockID);
