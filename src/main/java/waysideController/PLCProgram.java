...
public class PLCProgram extends AbstractParseTreeVisitor<Value> implements PLCVisitor<Value> {
  ...
  public Value visitIf_else_statement(PLCParser.If_else_statementContext ctx) {
    ...
    for (; index < ctx.getChildCount(); index++) {
      ...
      if (conditional && statement instanceof PLCParser.StatementContext) visit(statement);
      else if (statement.getText().equals("else")) break;
    }
    if (!conditional) {
      for (; index < ctx.getChildCount(); index++) {
        ParseTree statement = ctx.getChild(index);
        if (statement instanceof PLCParser.StatementContext) visit(statement);
      }
    }
    return new Value(conditional);
  }
  ...
  public Value visitOr_operator(PLCParser.Or_operatorContext ctx) {
    for (int i = 0; i < ctx.getChildCount(); i++) {
      ...
      if (child instanceof PLCParser.Single_valContext
          | child instanceof PLCParser.Compound_valueContext
          | child instanceof PLCParser.And_operatorContext) {
  ...
  public Value visitList_value(PLCParser.List_valueContext ctx) {
    ...
    switch (listName) {
      case "occupied":
        ...
        if (occupancyBlock != null) return new Value(occupancyBlock.isOccupied());
        else {
      ...
      case "switch":
        ...
        if (switchBlock != null) return new Value(switchBlock.getSwitchState());
        else {
      ...
      case "direction":
        if (!directionMap.containsKey(index)) directionMap.put(index, false);
        return new Value(directionMap.get(index));
      case "dir_assigned":
        if (!dir_assignedMap.containsKey(index)) dir_assignedMap.put(index, false);
