...
public class WaysideControllerManager {
  ...
  @FXML private TableView<WaysideBlockSubject> blockTable;
  @FXML private TableColumn<WaysideBlockSubject, Integer> blockTableIDColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> blockTableCircuitColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> blockTableLightsColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> blockTableCrossingColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> blockTableAuthColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> blockTableMaintenanceCol;
  @FXML private TableView<WaysideBlockSubject> switchTable;
  @FXML private TableColumn<WaysideBlockSubject, Integer> switchTableIDColumn;
  @FXML private TableColumn<WaysideBlockSubject, Boolean> switchTableStateColumn;
  @FXML private TableColumn<WaysideBlockSubject, Integer> switchTableBlockOutColumn;
  @FXML private CheckBox maintenanceModeCheckbox;
  @FXML private TextField plcFolderTextField;
  @FXML private TableView<File> plcFileTable;
  @FXML private TableColumn<File, String> plcFileNameColumn;
  @FXML private TableColumn<File, String> plcFileDateModifiedColumn;
  @FXML private Button plcFolderButton;
  @FXML private Button plcUploadButton;
  @FXML private ProgressBar uploadProgressBar;
  @FXML private Circle plcActiveIndicator;
  @FXML private Label plcCurrentFileLabel;
  @FXML private ComboBox<WaysideController> changeControllerComboBox;
  @FXML private Label changeControllerLabel;
  ...
  public void initialize() {
    ...
    plcFolderTextField.setOnAction(
        event -> updatePLCTableView(new File(plcFolderTextField.getText())));
    plcUploadButton.setOnAction(event -> uploadPLC());
    //        testBench.tbCreateNewControllerButton.setOnAction(event -> createNewController());
    changeControllerComboBox.setOnAction(
        event -> changeActiveController(changeControllerComboBox.getValue()));
    maintenanceModeCheckbox.setOnAction(
        event -> {
    ...
    blockTableIDColumn.setCellValueFactory(
        block -> block.getValue().getIntegerProperty(blockID_p).asObject());
    blockTableCircuitColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(occupied_p));
    //        blockTableLightsColumn.setCellValueFactory(block ->
    // block.getValue().getTrafficLightColor());
    blockTableLightsColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(lightState_p));
    blockTableCrossingColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(crossingState_p));
    blockTableAuthColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(authority_p));
    blockTableMaintenanceCol.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(inMaintenance_p));

    switchTableIDColumn.setCellValueFactory(
        block -> block.getValue().getIntegerProperty(switchBlockParent_p).asObject());
    switchTableBlockOutColumn.setCellValueFactory(
        block -> block.getValue().getIntegerProperty(switchedBlockID_p).asObject());
    switchTableStateColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(switchState_p));

    plcFileNameColumn.setCellValueFactory(
        file -> new ReadOnlyObjectWrapper<>(file.getValue().getName()));
    plcFileDateModifiedColumn.setCellValueFactory(
        file ->
            new ReadOnlyObjectWrapper<>(
                dateFormat.format(new Date(file.getValue().lastModified()))));
    plcFileTable.setOnMousePressed(
        mouseEvent -> {
          if (mouseEvent.isPrimaryButtonDown() && mouseEvent.getClickCount() >= 2) uploadPLC();
        });
  ...
  private void setupTableCellFactories() {
    blockTableCircuitColumn.setCellFactory(
        waysideBlockSubjectBooleanTableColumn ->
            new BooleanIconTableCell<>(null, "/Framework.GUI.Images/train_24.png", 24, 24));
    blockTableLightsColumn.setCellFactory(
        waysideBlockSubjectBooleanTableColumn -> {
          TableCell<WaysideBlockSubject, Boolean> tabelCell =
              new TernaryIconTableCell(
                  "/Framework.GUI.Images/Signal_Light_Red_24.png",
                  "/Framework.GUI.Images/Signal_Light_Green_24.png",
                  24,
                  24,
                  hasLight_p);
          tabelCell.setOnMouseClicked(
              event -> {
                if (currentSubject.getBooleanProperty(maintenanceMode_p).get()) {
                  if (tabelCell.getTableRow().getItem().getBlock().hasLight()) {
                    currentSubject
                        .getController()
                        .maintenanceSetTrafficLight(
                            tabelCell.getTableRow().getItem().getBlock().getBlockID(),
                            !tabelCell.getTableRow().getItem().getBlock().getLightState());
                  }
                }
              });
          return tabelCell;
        });
    blockTableCrossingColumn.setCellFactory(
        waysideBlockSubjectBooleanTableColumn -> {
          TableCell<WaysideBlockSubject, Boolean> tabelCell =
              new TernaryIconTableCell(
                  "/Framework.GUI.Images/Crossing_Down_24.png",
                  "/Framework.GUI.Images/Crossing_Up_24.png",
                  24,
                  24,
                  hasCrossing_p);
          tabelCell.setOnMouseClicked(
              event -> {
                if (currentSubject.getBooleanProperty(maintenanceMode_p).get()) {
                  if (tabelCell.getTableRow().getItem().getBlock().hasCrossing()) {
                    currentSubject
                        .getController()
                        .maintenanceSetCrossing(
                            tabelCell.getTableRow().getItem().getBlock().getBlockID(),
                            !tabelCell.getTableRow().getItem().getBlock().getCrossingState());
                  }
                }
              });
          return tabelCell;
        });
    blockTableAuthColumn.setCellFactory(
        column ->
            new TableCell<WaysideBlockSubject, Boolean>() {
              @Override
              public void updateItem(Boolean item, boolean empty) {
                ...
                if (empty || item == null) {
                  ...
                  CheckBox checkBox;
                  {
                    ...
                    checkBox.setDisable(
                        !currentSubject.getBooleanProperty(maintenanceMode_p).get());
                    checkBox.setOpacity(1.0);
                    checkBox.setOnAction(
                        event -> {
                          currentSubject
                              .getController()
                              .maintenanceSetAuthority(
                                  block.getBlock().getBlockID(), checkBox.isSelected());
                        });
                  }
                  setGraphic(checkBox);
                }
    ...
    blockTableMaintenanceCol.setCellFactory(
        waysideBlockSubjectBooleanTableColumn -> {
          TableCell<WaysideBlockSubject, Boolean> tabelCell =
              new BooleanIconTableCell<>(null, "/Framework.GUI.Images/wrench-icon_24.png", 24, 24);
          tabelCell.setOnMouseClicked(
              event -> {
                currentSubject
                    .getController()
                    .CTCChangeBlockMaintenanceState(
                        tabelCell.getTableRow().getItem().getBlock().getBlockID(),
                        !tabelCell.getTableRow().getItem().getBlock().inMaintenance());
              });
          return tabelCell;
        });
    switchTableStateColumn.setCellFactory(
        column ->
            new TableCell<WaysideBlockSubject, Boolean>() {
              @Override
              public void updateItem(Boolean item, boolean empty) {
  ...
  /**
   * Updates the block list and switch list in the GUI with the information from the current wayside
   * controller
   */
  ...
  private void changeActiveController(WaysideController controller) {
    ...
    if (currentSubject != null) {
      ...
      plcCurrentFileLabel
          .textProperty()
          .unbindBidirectional(currentSubject.getStringProperty(PLCName_p));
      plcActiveIndicator
          .fillProperty()
          .unbindBidirectional(currentSubject.getPaintProperty(activePLCColor_p));
      currentSubject
          .getBooleanProperty(maintenanceMode_p)
          .removeListener((observable, oldValue, newValue) -> updateMaintenanceWriteable());
    }
    ...
    // Bind new subject
    appendListener(
        currentSubject.getBooleanProperty(maintenanceMode_p),
        (observable, oldValue, newValue) -> updateMaintenanceWriteable());
    plcCurrentFileLabel
        .textProperty()
        .bindBidirectional(currentSubject.getStringProperty(PLCName_p));
    plcActiveIndicator
        .fillProperty()
        .bindBidirectional(currentSubject.getPaintProperty(activePLCColor_p));
