...
public class WaysideControllerHW implements PLCRunner {
  ...
  public WaysideControllerHW(String comPort) {
    ...
    port.setComPortTimeouts(
        SerialPort.TIMEOUT_READ_SEMI_BLOCKING, 0, 0); // block until bytes can be written
    port.openPort();
  ...
  private void setupBlocks(int[] blockIDList, String trackLine) {
    ...
    ConcurrentSkipListMap<Integer, BasicBlock> blockList =
        BasicBlockParser.getInstance().getBasicLine(Lines.valueOf(trackLine));
  ...
  protected void parseCOMMessage(String message) {
    if (!message.equals("runPLC")) System.out.println("Received: " + message);
    ...
    switch (values[0]) {
      ...
      case "occupancy" -> {
        ...
        blockMap
            .get(Integer.parseInt(setValues[0]))
            .setOccupied(Boolean.parseBoolean(setValues[1]));
      }
      case "switchState" -> {
        String[] setValues = values[1].split(":");
        blockMap
            .get(Integer.parseInt(setValues[0]))
            .setSwitchState(Boolean.parseBoolean(setValues[1]));
      }
      case "crossing" -> {
        String[] setValues = values[1].split(":");
        blockMap
            .get(Integer.parseInt(setValues[0]))
            .setCrossingState(Boolean.parseBoolean(setValues[1]));
      }
      case "trafficLight" -> {
        String[] setValues = values[1].split(":");
        blockMap
            .get(Integer.parseInt(setValues[0]))
            .setLightState(Boolean.parseBoolean(setValues[1]));
      }
      case "auth" -> {
        String[] setValues = values[1].split(":");
        blockMap
            .get(Integer.parseInt(setValues[0]))
            .setBooleanAuth(Boolean.parseBoolean(setValues[1]));
      }
      case "runPLC" -> {
  ...
  private void runPLC() {
    ...
    for (int changeIndex = 0; changeIndex < changeSize; changeIndex++) {
      ...
      //            System.out.println("PLC Change: " + change.changeType() + " " + change.blockID()
      // + " " + change.changeValue() + " " + plcResults[0].size());
