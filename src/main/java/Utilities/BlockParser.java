...
class BlockParser {
  ...
  public static ConcurrentHashMap<Lines, ConcurrentSkipListMap<Integer, BasicBlock>> parseCSV(
      String filePath) {
    ConcurrentHashMap<Lines, ConcurrentSkipListMap<Integer, BasicBlock>> map =
    ...
    try {
      ...
      for (int i = 1; i < lines.size(); i++) {
        ...
        line = lines.get(i);
        // if the values array is not the same size as the headers array, fill in the missing values
        // with empty strings
  ...
  public static BasicBlock fromCsv(String[] values, String[] headers) {
    ...
    double cumulativeElevation =
        Double.parseDouble(values[indexOf(headers, "CUMUALTIVE ELEVATION (M)")]);
    ...
    BasicBlock.NextBlock nextBlock =
        parseNextBlock(
            isSwitch,
            values[indexOf(headers, "North Bound")],
            values[indexOf(headers, "South Bound")]);
    ...
    return new BasicBlock(
        ...
        isLight,
        blockType,
        Optional.ofNullable(stationName),
        doorDirection,
        nextBlock);
  }
  ...
  private static NextBlock parseNextBlock(
      boolean isSwitch, String northBoundString, String southBoundString) {
