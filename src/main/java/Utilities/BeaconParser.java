...
public class BeaconParser {
  ...
  /**
   * A map of switch block numbers to the indices(directional next blocks) of the beacons that are
   * created for that switch
   */
  ...
  private static void createBeacons(
  ...
  private static void createBeaconsForSwitch(
      BasicBlock switchBlock,
      BasicTrackLine trackLine,
    ...
    createBeaconForPath(
        ...
        trackLine,
        beacons,
        switchBlock.nextBlock().northDefault(),
        Direction.NORTH);
    createBeaconForPath(
        switchBlock,
        switchBlockNumber,
        trackLine,
        beacons,
        switchBlock.nextBlock().southDefault(),
        Direction.SOUTH);
    createBeaconForPath(
        switchBlock,
        switchBlockNumber,
        trackLine,
        beacons,
        switchBlock.nextBlock().northAlternate(),
        Direction.NORTH);
    createBeaconForPath(
        switchBlock,
        switchBlockNumber,
        trackLine,
        beacons,
        switchBlock.nextBlock().southAlternate(),
        Direction.SOUTH);
  }

  private static void createBeaconForPath(
    ...
    int endBlockNumber =
        traverseTrack(startConnection.blockNumber(), trackLine, blockIndices, startDirection);
  ...
  private static void createBeaconsForStation(
      BasicBlock stationBlock,
      BasicTrackLine trackLine,
      ConcurrentHashMap<Integer, Beacon> beacons) {
    ...
    createBeaconForPath(
        ...
        trackLine,
        beacons,
        stationBlock.nextBlock().south(),
        Direction.SOUTH);
  }

  private static int traverseTrack(
