...
public class BooleanIconTableCell<T> extends TableCell<T, Boolean> {
  ...
  /**
   * Create a new TableCell with two icons to display for true and false values. Providing a null
   * value for either icon path will result in no icon being displayed for that value.
   *
   * @param falseIconPath path to the icon to display for false values (null or empty string for no
   *     icon)
   * @param trueIconPath path to the icon to display for true values (null or empty string for no
   *     icon)
