...
public class TrackBlock {
  ...
  public TrackBlock(BasicBlock blockInfo) {
    ...
    if (isSwitch && blockType == BlockType.STATION) {
      ...
      this.feature =
          new SwitchStationBlock(
              blockInfo.stationName().get(),
              blockInfo.doorDirection().get(),
              blockInfo.nextBlock());
    } else if (isSwitch) {
      this.northConnect = null;
      this.southConnect = null;
      ...
      this.feature =
          new StationBlock(blockInfo.stationName().get(), blockInfo.doorDirection().get());
  ...
  void addOccupation(TrainModel train) {
    if (!occupied) {
      ...
      logger.warn(
          "TrackBlock: {} is already occupied by Train: {} ", blockID, occupiedBy.getTrainNumber());
    }
  }
      ...
      if (blockInfo.stationName().isEmpty() || blockInfo.doorDirection().isEmpty()) {
        throw new IllegalArgumentException(
            "Block: "
                ...
                + "Station block must have a station name and door direction");
      }
    } else if (blockInfo.isSwitch()) {
      if (blockInfo.nextBlock().northDefault() == null
          || blockInfo.nextBlock().southDefault() == null) {
        throw new IllegalArgumentException(
            "Block: "
                + blockInfo.blockNumber()
                + "Switch block must have a default connection for both directions.");
      }
      if (blockInfo.nextBlock().northAlternate() == null
          && blockInfo.nextBlock().southAlternate() == null) {
        throw new IllegalArgumentException(
            "Block: "
                + blockInfo.blockNumber()
                + "Switch block must have an alternate connection for at least one direction.");
      }
    }
  }
