...
public class TrackModelManager {
  ...
  @FXML public Label ticketSalesLabel, locationLabel, passDisembarkLabel, passEmbarkedLabel;
  @FXML public Label tempLabel, switchBlockNumbersLabel;
  @FXML public Label switchStateLabel, signalBlockNumberLabel, signalStateLabel, crossingLabel;
  @FXML public Label beaconInfoLabel, lineInfoLabel, degF, sectionsLabel, logo;
  @FXML public TitledPane sssSec, trackHeaterSec, murphySec, beaconInfoSec;
  @FXML public Tab switchTab, signalTab, stationTab;
  @FXML public TabPane sssTabs;

  // add line

  @FXML public TextField lineNameInput, trackFilePath;
  @FXML public Button chooseFile, trackUpload;

  // murphy
  @FXML private ComboBox<String> chooseFailureMode;
  @FXML private Button murphyEnter;

  // track heater and environmental temperature
  @FXML private Label trackHeaterStatus;
  @FXML private Label outsideTemp;

  // station signal switch
  @FXML private Label nameOfStationLabel, passEmbarkedValue, passDisembarkedValue;
  @FXML private Label ticketSalesValue, signalStateDisplay, signalBlockNumberDisplay;

  @FXML private ChoiceBox<String> pickLine;

  // beacon information
  @FXML private Label displayBeaconInfo, beaconBlockNumber;

  // switch information
  @FXML private Label switchStateDisplay, switchBlockNumbersDisplay, crossingState;

  // table
  @FXML private TableView<TrackBlockSubject> lineTable;
  @FXML private TableColumn<TrackBlockSubject, Boolean> directionColumn;
  @FXML private TableColumn<TrackBlockSubject, String> failureColumn;
  @FXML private TableColumn<TrackBlockSubject, Integer> blockColumn;
  @FXML private TableColumn<TrackBlockSubject, Double> lengthColumn;
  @FXML private TableColumn<TrackBlockSubject, Boolean> occupiedColumn;
  @FXML
  ...
  public void initialize() {
    ...
    chooseFailureMode
        .getItems()
        .addAll("Broken Rail", "Track Circuit Failure", "Power Failure", "Fix Track Failure");
  ...
  private void initializeTable() {
    ...
    blockColumn.setCellValueFactory(
        cellData -> cellData.getValue().blockNumberProperty().asObject());
    lengthColumn.setCellValueFactory(
        cellData -> cellData.getValue().blockLengthProperty().asObject());
    gradeColumn.setCellValueFactory(
        cellData -> cellData.getValue().blockGradeProperty().asObject());
    elevationColumn.setCellValueFactory(
        cellData -> cellData.getValue().blockElevationProperty().asObject());
    speedLimitColumn.setCellValueFactory(
        cellData -> cellData.getValue().speedLimitProperty().asObject());
    ...
    occupiedColumn.setCellFactory(
        TrackLineSubjectBooleanTableColumn ->
            new BooleanIconTableCell<>(
                "/Framework.GUI.Images/train_rail_24.png",
                "/Framework.GUI.Images/train_24.png",
                24,
                24));
    directionColumn.setCellFactory(
        TrackLineSubjectBooleanTableColumn -> {
          return new DirectionTableCell(
              "/Framework.GUI.Images/down_arrow.png", "/Framework.GUI.Images/up_arrow.png", 24, 24);
        });
  ...
  public void selectBlock(TrackBlockSubject newSubject) {
    ...
    if (currentBlockSubject != null) {
      ...
      if (currentBlockSubject.isIsStation()) {
        passEmbarkedValue
            .textProperty()
            .unbindBidirectional(currentBlockSubject.passEmbarkedProperty());
        passDisembarkedValue
            .textProperty()
            .unbindBidirectional(currentBlockSubject.passDisembarkedProperty());
        ticketSalesValue
            .textProperty()
            .unbindBidirectional(currentBlockSubject.ticketSalesProperty());
        nameOfStationLabel
            .textProperty()
            .unbindBidirectional(currentBlockSubject.nameOfStationProperty());
      }

      if (currentBlockSubject.isIsSwitch()) {
        switchBlockNumbersDisplay
            .textProperty()
            .unbindBidirectional(currentBlockSubject.switchBlockIDProperty());
        switchStateDisplay
            .textProperty()
            .unbindBidirectional(currentBlockSubject.switchStateProperty());
        switchBlockNumbersDisplay
            .textProperty()
            .unbindBidirectional(currentBlockSubject.switchBlockIDProperty());
      }
      ...
      if (currentBlockSubject.isIsSignal()) {
        signalStateDisplay
            .textProperty()
            .unbindBidirectional(currentBlockSubject.signalStateProperty());
        signalBlockNumberDisplay
            .textProperty()
            .unbindBidirectional(currentBlockSubject.signalIDProperty());
      }

      if (currentBlockSubject.isIsCrossing()) {
        crossingState
            .textProperty()
            .unbindBidirectional(currentBlockSubject.crossingStateProperty());
      }

      if (currentBlockSubject.isIsBeacon()) {
        displayBeaconInfo
            .textProperty()
            .unbindBidirectional(currentBlockSubject.setBeaconProperty());
      ...
      elevationColumn
          .textProperty()
          .unbindBidirectional(currentBlockSubject.blockElevationProperty().asObject());
      occupiedColumn.textProperty().unbindBidirectional(currentBlockSubject.isOccupiedProperty());
      gradeColumn
          .textProperty()
          .unbindBidirectional(currentBlockSubject.blockGradeProperty().asObject());
      ...
      trackHeaterStatus
          .textProperty()
          .unbindBidirectional(currentBlockSubject.trackHeaterProperty());
    }
    ...
    if (currentBlockSubject.isIsStation()) {
      passEmbarkedValue
          .textProperty()
          .bindBidirectional(currentBlockSubject.passEmbarkedProperty());
      passDisembarkedValue
          .textProperty()
          .bindBidirectional(currentBlockSubject.passDisembarkedProperty());
      ticketSalesValue.textProperty().bindBidirectional(currentBlockSubject.ticketSalesProperty());
      nameOfStationLabel
          .textProperty()
          .bindBidirectional(currentBlockSubject.nameOfStationProperty());
    } else {
    ...
    if (currentBlockSubject.isIsSwitch()) {
      switchStateDisplay
          .textProperty()
          .bindBidirectional(currentBlockSubject.switchStateProperty());
      switchBlockNumbersDisplay
          .textProperty()
          .bindBidirectional(currentBlockSubject.switchBlockIDProperty());
    } else {
    ...
    if (currentBlockSubject.isIsSignal()) {
      signalStateDisplay
          .textProperty()
          .bindBidirectional(currentBlockSubject.signalStateProperty());
  ...
  private void murphyEnter() {
    ...
    switch (failure) {
      ...
      case "Track Circuit Failure":
        ...
        currentBlockSubject.setIsOccupied(true);
        TrackLineMap.getTrackLine(line)
            .setTrackCircuitFailure(currentBlockSubject.getBlockNumber(), true);
        break;
      case "Power Failure":
