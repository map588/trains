...
public class TrainModelImpl implements TrainModel, Notifier {
  ...
  private void initializeValues() {
    ...
    this.mass =
        (Constants.EMPTY_TRAIN_MASS * numCars)
            + (Constants.PASSENGER_MASS * (crewCount + numPassengers));
  ...
  public void trainModelTimeStep(Future<UpdatedTrainValues> updatedTrainValuesFuture)
  ...
  // Method to update all physics of the train model, including mass, acceleration, speed, distance,
  // and temperature
  private void physicsUpdate() {
    // MASS CALCULATION (some redundancy here, the empty train mass is final, and the crew count is
    // final)
    ...
    if (this.power <= MAX_POWER_W * numCars) {
      this.speed =
          (this.speed + (this.TIME_DELTA / 2) * (this.acceleration + previousAcceleration));
    }
    ...
    // TODO: If we can go faster than the "maximum speed" in simulation, we are doing something
    // wrong
  ...
  public void setSetTemperature(double temp) {
    ...
    notifyChange(
        SETTEMPERATURE_PROPERTY,
        Conversion.convertTemperature(this.setTemperature, CELSIUS, FAHRENHEIT));
  }

  public void setRealTemperature(double temp) {
    this.realTemperature = temp;
    notifyChange(
        REALTEMPERATURE_PROPERTY,
        Conversion.convertTemperature(this.realTemperature, CELSIUS, FAHRENHEIT));
  }

  public void setAcceleration(double acceleration) {
    this.acceleration = acceleration;
    notifyChange(
        ACCELERATION_PROPERTY, Conversion.convertAcceleration(this.acceleration, MPS2, FPS2));
  }
  ...
  public void setDistanceTraveled(double distance) {
    ...
    notifyChange(
        DISTANCETRAVELED_PROPERTY, Conversion.convertDistance(this.distanceTraveled, METERS, FEET));
  }
  ...
  public void updatePassengers() {
    ...
    this.setMass(
        (Constants.EMPTY_TRAIN_MASS * this.numCars)
            + (Constants.PASSENGER_MASS * (this.crewCount + this.numPassengers)));
  }
  ...
  public void setValue(String propertyName, Object newValue) {
    ...
    switch (propertyName) {
        ...
        // case Properties.COMMANDSPEED_PROPERTY -> this.commandSpeed =
        // convertVelocity((double)newValue, MPH, MPS);
        // case Properties.ACTUALSPEED_PROPERTY -> this.speed = convertVelocity((double)newValue,
        // MPH, MPS);
        // case Properties.ACCELERATION_PROPERTY -> this.acceleration =
        // convertAcceleration((double)newValue, FPS2, MPS2);
        // case Properties.POWER_PROPERTY -> this.power = convertPower((double)newValue, HORSEPOWER,
        // WATTS);
      ...
      case Properties.EMERGENCYBRAKE_PROPERTY -> {
        this.emergencyBrake = (boolean) newValue;
        this.controller.setPassengerEBrake();
      }
      ...
      case Properties.SIGNALFAILURE_PROPERTY -> this.signalFailure = (boolean) newValue;
        // case Properties.SETTEMPERATURE_PROPERTY -> this.setTemperature =
        // convertTemperature((double)newValue, FAHRENHEIT, CELSIUS);
        // case Properties.REALTEMPERATURE_PROPERTY -> this.realTemperature =
        // convertTemperature((double)newValue, FAHRENHEIT, CELSIUS);
        ...
        // case Properties.DISTANCETRAVELED_PROPERTY -> this.distanceTraveled =
        // convertDistance((double)newValue, FEET, METERS);
        // case Properties.LENGTH_PROPERTY -> this.length = convertDistance((double)newValue, FEET,
        // METERS);
    }
  }
