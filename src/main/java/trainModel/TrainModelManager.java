...
public class TrainModelManager {
  ...
  @FXML public Button eBrakeBtn;
  @FXML public ChoiceBox<Integer> trainDropDown;
  @FXML public ToggleButton brakeFailureBtn, powerFailureBtn, signalFailureBtn;
  @FXML
  public Label gradeLabel,
      maxPowerLabel,
      medAccelerationLabel,
      maxVelocityLabel,
      trainLengthLabel,
      trainHeightLabel,
      trainWidthLabel,
      numCarsLabel;
  @FXML public Label numPassengerLabel, crewCountLabel, massLabel;
  @FXML
  public Gauge actualPowerDisp,
      actualVelocityDisp,
      actualAccelerationDisp,
      cmdSpeedDisp,
      authorityDisp,
      setTempDisp,
      realTempDisp;
  @FXML public Circle extLightsEn, intLightsEn, leftDoorsEn, rightDoorsEn, sBrakeEn, eBrakeEn;
  @FXML public AnchorPane masterTrainModelPane;
  ...
  static final ReentrantLock subjectChangeLock = new ReentrantLock();
  ...
  private void bindLabelToProperty(String property, Label label) {
    appendListener(
        ...
        (obs, oldValue, newValue) -> {
          ...
          try {
            if (property.equals(MASS_PROPERTY)
                || property.equals(LENGTH_PROPERTY)
                || property.equals(GRADE_PROPERTY)) {
  ...
  private void bindIndicators() {
    appendListener(
        subject.getBooleanProperty(EMERGENCYBRAKE_PROPERTY),
        (obs, oldSelection, newSelection) -> updateEBrakeIndicator(newSelection));
    appendListener(
        subject.getBooleanProperty(SERVICEBRAKE_PROPERTY),
        (obs, oldSelection, newSelection) -> updateSBrakeIndicator(newSelection));
    appendListener(
        subject.getBooleanProperty(EXTLIGHTS_PROPERTY),
        (obs, oldSelection, newSelection) -> updateExtLightsIndicator(newSelection));
    appendListener(
        subject.getBooleanProperty(INTLIGHTS_PROPERTY),
        (obs, oldSelection, newSelection) -> updateIntLightsIndicator(newSelection));
    appendListener(
        subject.getBooleanProperty(LEFTDOORS_PROPERTY),
        (obs, oldSelection, newSelection) -> updateLeftDoorsIndicator(newSelection));
    appendListener(
        subject.getBooleanProperty(RIGHTDOORS_PROPERTY),
        (obs, oldSelection, newSelection) -> updateRightDoorsIndicator(newSelection));
  }

  private void bindControls() {
    eBrakeBtn.setOnAction(event -> subject.setProperty(EMERGENCYBRAKE_PROPERTY, true));

    brakeFailureBtn.setOnAction(
        event -> {
  ...
  private void setupMapChangeListener() {
    ...
    // Create a listener that reacts to any change (add, remove, update) by updating choice box
    // items
    ObservableHashMap.MapListener<Integer, TrainModelSubject> genericListener =
        new ObservableHashMap.MapListener<>() {
          ...
          public void onRemoved(Integer key, TrainModelSubject value) {
            Platform.runLater(
                () -> {
                  updateChoiceBoxItems();
                  // Additional logic can be added here to select another item if the current
                  // selection was removed
                });
          }

          public void onUpdated(
              Integer key, TrainModelSubject oldValue, TrainModelSubject newValue) {
            Platform.runLater(
                () -> {
                  updateChoiceBoxItems();
  ...
  private void setUpCircleColors() {
    List<Circle> circleList =
        Arrays.asList(extLightsEn, intLightsEn, leftDoorsEn, rightDoorsEn, sBrakeEn, eBrakeEn);
