...
public class TrainModelTB {
  @FXML public ToggleButton tbSBrake, tbIntLights, tbExtLights, tbLeftDoors, tbRightDoors;
  public Button tbTimeButton, tbEBrake;
  @FXML public TextField tbPower, tbSpeed, tbAuthority, tbGrade, tbTemp, tbTimeDelta;
  @FXML public Circle tbBrakeFailure, tbPowerFailure, tbSignalFailure;
  ...
  private void bindTrain() {
    ...
    tbPower.setOnAction(
        event -> subject.setProperty("power", Double.parseDouble(tbPower.getText())));
    tbSpeed.setOnAction(
        event -> subject.setProperty("commandSpeed", Double.parseDouble(tbSpeed.getText())));
    tbAuthority.setOnAction(
        event ->
            subject.setProperty(
                Properties.AUTHORITY_PROPERTY, Integer.parseInt(tbAuthority.getText())));
    tbGrade.setOnAction(
        event -> subject.setProperty("grade", Double.parseDouble(tbGrade.getText())));
    tbTemp.setOnAction(
        event -> subject.setProperty("setTemperature", Double.parseDouble(tbTemp.getText())));
    tbTimeDelta.setOnAction(
        event -> subject.setProperty("timeDelta", Integer.parseInt(tbTimeDelta.getText())));

    appendListener(
        subject.getBooleanProperty("brakeFailure"),
        (observable, oldValue, newValue) -> updateBrakeFailureIndicator(newValue));
    appendListener(
        subject.getBooleanProperty("powerFailure"),
        (observable, oldValue, newValue) -> updatePowerFailureIndicator(newValue));
    appendListener(
        subject.getBooleanProperty("signalFailure"),
        (observable, oldValue, newValue) -> updateSignalFailureIndicator(newValue));
  }
