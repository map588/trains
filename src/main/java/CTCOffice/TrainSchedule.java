...
public class TrainSchedule implements Notifier {
  ...
  private static final double KmphToMps = 0.277778;
  private static final double minSpeedLimit = 4.1;
  ...
  public TrainSchedule(
      int trainID, String line, int dispatchTime, int carCount, ArrayList<TrainStop> stops) {
  ...
  public int checkSchedule() {
    ...
    if (!stops.isEmpty()) {
      if (stops.get(stops.size() - 1).getStationBlockID()
          != TrackLayout.get(TrackLayout.size() - 1)) {
  ...
  public void setAuthorities() {
    ...
    for (TrainStop stop : stops) {
      ...
      double metersAuthority =
          (blockSubjectMap
                  .getSubject(
                      BlockIDs.of(TrackLayout.get(station), Enum.valueOf(Lines.class, line)))
                  .getBlockInfo()
                  .getLength()
              / 2);
      ...
      for (int j = visited; j < TrackLayout.indexOf(stop.getStationBlockID()); j++) {
        double blockLength =
            blockSubjectMap
                .getSubject(BlockIDs.of(TrackLayout.get(j), Enum.valueOf(Lines.class, line)))
                .getBlockInfo()
                .getLength();
        metersAuthority += blockLength;
        // System.out.println("Adding " + blockLength + " to authority of stop " +
        // stop.getStopIndex() + " of train " + trainID);
      }
      for (int j = 0; j <= blocksAuthority; j++) {
        double blockDistance =
            blockSubjectMap
                .getSubject(
                    BlockIDs.of(TrackLayout.get(visited + j), Enum.valueOf(Lines.class, line)))
                .getBlockInfo()
                .getLength();
    ...
    if (stops.get(0).getAuthorityList().get(0)
        < (stops.get(0).getArrivalTime() - dispatchTime) * minSpeedLimit) {
      stops
          .get(0)
          .setArrivalTime(dispatchTime + (stops.get(0).getAuthorityList().get(0) / minSpeedLimit));
      stops.get(0).setDepartureTime(stops.get(0).getArrivalTime() + 60);
    }
    for (int i = 1; i < stops.size(); i++) {
      if (stops.get(i).getAuthorityList().get(0)
          < ((stops.get(i).getArrivalTime() - stops.get(i - 1).getDepartureTime())
              * minSpeedLimit)) {
        stops
            .get(i)
            .setArrivalTime(
                stops.get(i - 1).getDepartureTime()
                    + (stops.get(i).getAuthorityList().get(0) / minSpeedLimit));
        stops.get(i).setDepartureTime(stops.get(i).getArrivalTime() + 60);
      }
    }
  }
    ...
    for (TrainStop stop : stops) {
        ...
        stop.getSpeedList()
            .add(
                (blockSubjectMap
                        .getSubject(
                            BlockIDs.of(
                                stop.getRoutePath().get(j), Enum.valueOf(Lines.class, line)))
                        .getBlockInfo()
                        .getSpeedLimit()
                    * KmphToMps));
        minTraversalTime +=
      ...
      if (scheduledTraversalTime < minTraversalTime) {
        if (stop.getStopIndex() == 0) {
          ...
          stop.setArrivalTime(
              stops.get(stop.getStopIndex() - 1).getDepartureTime() + minTraversalTime);
        ...
        double metersCoveredOverSpeedLimit =
            stop.getAuthorityList().get(0) - metersCoveredUnderSpeedLimit;
        double timeCoveredOverSpeedLimit = scheduledTraversalTime - timeUnderSpeedLimit;
        for (int j = 0; j < stop.getRoutePath().size(); j++) {
          CTCBlock block =
              CTCBlockSubjectMap.getInstance()
                  .getSubject(BlockIDs.of(stop.getRoutePath().get(j), Lines.getLine(line)))
                  .getBlockInfo();
          if (stop.getSpeedList().get(j) >= 0.0000) {
            if ((block.getLength() / (block.getSpeedLimit() * KmphToMps))
                >= (timeCoveredOverSpeedLimit
                    - ((metersCoveredOverSpeedLimit - block.getLength()) / averageSpeed))) {
              stop.getSpeedList().set(j, block.getSpeedLimit() * KmphToMps);
              metersCoveredOverSpeedLimit -= block.getLength();
              timeCoveredOverSpeedLimit -=
                  (block.getLength() / (block.getSpeedLimit() * KmphToMps));
            } else {
              stop.getSpeedList()
                  .set(
                      j,
