    ...
    public void saveScheduleFile(String fileName, ScheduleFile scheduleFile){
        ...
        for(int i = 1; i <= scheduleFile.getMultipleTrainSchedules().size(); i++){
            ...
            for(int j = 0; j < scheduleFile.getMultipleTrainSchedules().get(i).getStops().size(); j++) {
                ...
                for (int k = 0; k < scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getRoutePath().size(); k++) {
                    routePath.add(scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getRoutePath().get(k));
                    speedList.add(scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getSpeedList().get(k));
                    authorityList.add(scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getAuthorityList().get(k));
                }
                compressedStops.add(new CompressedStop(scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getStationBlockID(),
                        scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getArrivalTime(),
                        scheduleFile.getMultipleTrainSchedules().get(i).getStops().get(j).getDepartureTime(),
                        speedList, routePath, authorityList));
            }
            compressedTrainSchedules.add(new CompressedTrainSchedule(scheduleFile.getMultipleTrainSchedules().get(i).getTrainID(),
