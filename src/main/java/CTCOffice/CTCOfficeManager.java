...
/**
 * This class manages the GUI for the Centralized Traffic Control (CTC) office. It contains FXML
 * annotated fields for the various GUI components and methods for handling user interactions.
 */
public class CTCOfficeManager {
  ...
  /**
   * Because these color properties are only relevant to the GUI, they are not stored in the
   * CTCBlockSubject. They reflect boolean properties of the CTCBlockSubject, and are updated by
   * listeners.
   */
  ...
  /**
   * Initializes the GUI components. This method is called after all the FXML annotated fields have
   * been injected.
   */
  ...
  private void LineTableSet(
      TableColumn<CTCBlockSubject, Integer> blockNumberColumn,
      TableView<CTCBlockSubject> blockTable,
      TableColumn<CTCBlockSubject, Boolean> occupationLightColumn,
      TableColumn<CTCBlockSubject, String> switchStateColumn,
      TableColumn<CTCBlockSubject, Paint> switchLightColumn,
      TableColumn<CTCBlockSubject, Paint> crossingStateColumn,
      TableColumn<CTCBlockSubject, Paint> underMaintenanceColumn) {
    blockNumberColumn.setCellValueFactory(
        block ->
            new ReadOnlyObjectWrapper<>(
                block.getValue().getIntegerProperty(BLOCK_ID_PROPERTY).getValue()));
    ...
    occupationLightColumn.setCellValueFactory(
        block -> block.getValue().getBooleanProperty(OCCUPIED_PROPERTY));
    ...
    switchStateColumn.setCellValueFactory(
        block -> {
  ...
  private void switchColorListener(CTCBlockSubject block) {
    if (block.hasLight()) {
      switchColors.computeIfAbsent(block, k -> new SimpleObjectProperty<>()).setValue(Color.RED);
    } else {
      switchColors.computeIfAbsent(block, k -> new SimpleObjectProperty<>());
    }

    block
        .getBooleanProperty(SWITCH_LIGHT_STATE_PROPERTY)
        .addListener(
            (observable, oldValue, newValue) -> {
              boolean hasLight = block.hasLight();
              if (newValue && hasLight) switchColors.get(block).setValue(Color.GREEN);
              else if (hasLight) {
  ...
  private void crossingColorListener(CTCBlockSubject block) {
    ...
    if (block.hasCrossing()) {
      crossingColors
          .computeIfAbsent(block, k -> new SimpleObjectProperty<>())
          .setValue(Color.DARKGRAY);
    } else {
      crossingColors.computeIfAbsent(block, k -> new SimpleObjectProperty<>());
    }

    block
        .getBooleanProperty(CROSSING_STATE_PROPERTY)
        .addListener(
            (observable, oldValue, newValue) -> {
              boolean hasCrossing = block.hasCrossing();
              if (newValue && hasCrossing) crossingColors.get(block).setValue(Color.RED);
              else if (hasCrossing) {
  ...
  private void maintenanceColorListener(CTCBlockSubject block) {
    ...
    block
        .getBooleanProperty(UNDER_MAINTENANCE_PROPERTY)
        .addListener(
            (observable, oldValue, newValue) -> {
              if (newValue) maintenanceColors.get(block).setValue(Color.RED);
              else {
  ...
  private void setupLineTables() {
    ...
    for (CTCBlockSubject block : blockMap.getSubjects().values()) {
      ...
      if (!blockSelection
          .getItems()
          .contains(block.getIntegerProperty(BLOCK_ID_PROPERTY).getValue())) {
    ...
    LineTableSet(
        blockNumberColumnGreen,
        blockTableGreen,
        occupationLightColumnGreen,
        switchStateColumnGreen,
        switchLightColumnGreen,
        crossingStateColumnGreen,
        underMaintenanceColumnGreen);
    LineTableSet(
        blockNumberColumnRed,
        blockTableRed,
        occupationLightColumnRed,
        switchStateColumnRed,
        switchLightColumnRed,
        crossingStateColumnRed,
        underMaintenanceColumnRed);

    // Table editing bar
    switchLightToggle.setOnAction(
        event -> {
          // System.out.println("toggled traffic light for block " + blockSelection.getValue() + "
          // on line " + lineSelection.getValue() + "\n");
          CTCBlockSubject block =
              blockMap.getSubject(
                  BlockIDs.of(
                      blockSelection.getValue(),
                      Enum.valueOf(Lines.class, lineSelection.getValue())));
          block
              .getBlockInfo()
              .setSwitchLightState(true, !block.getBlockInfo().getSwitchLightState());
        });
    switchStateToggle.setOnAction(
        event -> {
          // System.out.println("toggled switch for block " + blockSelection.getValue() + " on line
          // " + lineSelection.getValue() + "\n");
          CTCBlockSubject block =
              blockMap.getSubject(
                  BlockIDs.of(
                      blockSelection.getValue(),
    ...
    crossingStateToggle.setOnAction(
        event -> {
          // System.out.println("toggled crossing for block " + blockSelection.getValue() + " on
          // line " + lineSelection.getValue() + "\n");
          CTCBlockSubject block =
              blockMap.getSubject(
                  BlockIDs.of(
                      blockSelection.getValue(),
    ...
    maintenanceToggle.setOnAction(
        event -> {
          // System.out.println("toggled maintenance for block " + blockSelection.getValue() + " on
          // line " + lineSelection.getValue() + "\n");
          CTCBlockSubject block =
              blockMap.getSubject(
                  BlockIDs.of(
          ...
          block
              .getBlockInfo()
              .setUnderMaintenance(true, !block.getBlockInfo().getUnderMaintenance());
        });
  ...
  private void setupScheduleTables() {
    ...
    scheduleNameColumn.setCellValueFactory(
        schedule -> schedule.getValue().getStringProperty(SCHEDULE_FILE_NAME_PROPERTY));
    scheduleDateModColumn.setCellValueFactory(
        schedule -> schedule.getValue().getStringProperty(LAST_MODIFIED_PROPERTY));
    trainNumColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getIntegerProperty(NUM_TRAINS_PROPERTY).getValue()));
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (newValue != null) {
                ...
                // newValue.getProperty(SCHEDULE_FILE_NAME_PROPERTY).getValue() + "\n");
                trainSelectTable.getItems().clear();
                for (int i = 1; i <= newValue.getSchedule().getNumTrains(); i++) {
                  trainSelectTable
                      .getItems()
                      .add(newValue.getSchedule().getTrainSchedule(i).getSubject());
                  trainIDSelector.getItems().add(i);
                }
              }
            });

    scheduleSearchField.setOnAction(
        event -> {
    ...
    scheduleBrowseButton.setOnAction(
        event -> {
    ...
    scheduledTrainColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getIntegerProperty(TRAIN_ID_PROPERTY).getValue()));
    lineColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getStringProperty(LINE_PROPERTY).getValue()));
    dispatchTimeColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getStringProperty(DISPATCH_TIME_PROPERTY).getValue()));
    carNumberColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getIntegerProperty(CAR_COUNT_PROPERTY).getValue()));

    lineTrainSelector.getItems().add(Lines.GREEN.toString());
    lineTrainSelector.getItems().add(Lines.RED.toString());
    ...
    dispatchTimeSelector.setOnAction(
        event -> {
          if (!dispatchTimeSelector.getText().matches("\\d{0,2}:\\d{0,2}")) {
            dispatchTimeSelector.setText(
                convertDoubleToClockTime(
                    scheduleEditTable
                            .getItems()
                            .get(
                                trainSelectTable
                                        ...
                                        .getStopCount()
                                    - 1)
                            .getIntegerProperty(DEPARTURE_TIME_PROPERTY)
                            .getValue()
                        + 5.0));
          }
        });
    AddTrain.setOnAction(
        event -> {
          //  System.out.println("added train to schedule " +
          // scheduleTable.getSelectionModel().selectedItemProperty().getValue().getProperty(SCHEDULE_FILE_NAME_PROPERTY).getValue() + "\n");
          ScheduleFile schedule = scheduleTable.getSelectionModel().getSelectedItem().getSchedule();
          schedule.putTrainSchedule(
              schedule.getMultipleTrainSchedules().size() + 1,
              new TrainSchedule(
                  schedule.getMultipleTrainSchedules().size() + 1,
                  lineTrainSelector.getValue(),
                  (int) convertClockTimeToDouble(dispatchTimeSelector.getText()),
                  carsSelector.getValue(),
                  new ArrayList<>()));
          trainSelectTable
              .getItems()
              .add(
                  schedule
                      .getMultipleTrainScheduleSubjects()
                      .get(schedule.getMultipleTrainSchedules().size()));
          trainIDSelector.getItems().add(schedule.getMultipleTrainSchedules().size());
        });
    RemoveTrain.setOnAction(
        event -> {
          // System.out.println("removed train from schedule " +
          // scheduleTable.getSelectionModel().selectedItemProperty().getValue().getProperty(SCHEDULE_FILE_NAME_PROPERTY).getValue() + "\n");
          ScheduleFile schedule = scheduleTable.getSelectionModel().getSelectedItem().getSchedule();
          schedule.removeTrainSchedule(trainIDSelector.getValue());
          trainSelectTable
              .getItems()
              .remove(trainSelectTable.getSelectionModel().getSelectedItem());
          trainIDSelector.getItems().remove(trainIDSelector.getValue());
        });
    saveTrainButton.setOnAction(
        event -> {
          // System.out.println("saved train to schedule " +
          // selectedSchedule.getSubject().getProperty(SCHEDULE_FILE_NAME_PROPERTY).getValue() +
          // "\n");
          TrainScheduleSubject train =
              selectedSchedule.getTrainSchedule(trainIDSelector.getValue()).getSubject();
  ...
  private void setupStopTable() {
    ...
    stopIndexColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getIntegerProperty(STOP_INDEX_PROPERTY).getValue()));
    stationBlockIDColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getIntegerProperty(DESTINATION_PROPERTY).getValue()));
    arrivalTimeColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getStringProperty(ARRIVAL_TIME_PROPERTY).getValue()));
    departureTimeColumn.setCellValueFactory(
        schedule ->
            new ReadOnlyObjectWrapper<>(
                schedule.getValue().getStringProperty(DEPARTURE_TIME_PROPERTY).getValue()));
    stationStopSelector.setText(
        (selectedTrain.getLine().equals("GREEN") ? GreenTrackLayout : RedTrackLayout)
            .get(0)
            .toString());
    scheduleEditTable
        .getSelectionModel()
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (newValue != null) {
                ...
                arrivalTimeSelector.setText(
                    convertDoubleToClockTime(newValue.getStop().getArrivalTime()));
                departureTimeSelector.setText(
                    convertDoubleToClockTime(newValue.getStop().getDepartureTime()));
              }
            });
          ...
          row.setOnDragDropped(
              event -> {
                ...
                if (db.hasContent(SERIALIZED_MIME_TYPE)) {
                  ...
                  event.consume();
                  TrainSchedule train =
                      selectedSchedule.getTrainSchedule(trainIDSelector.getValue());
        ...
        event -> {
          ...
          if (stationStopSelector.getText().isEmpty()) {
            if (train.getStopCount() <= 0) {
              stationStopSelector.setText(
                  ""
                      + (selectedTrain.getLine().equals("GREEN")
                              ? GreenTrackLayout
                              : RedTrackLayout)
                          .get(3));
            } else if ((selectedTrain.getLine().equals("GREEN") ? GreenTrackLayout : RedTrackLayout)
                    .size()
                > train.getStopCount() * 5) {
              stationStopSelector.setText(
                  ""
                      + (selectedTrain.getLine().equals("GREEN")
                              ? GreenTrackLayout
                              : RedTrackLayout)
                          .get(train.getStopCount() * 5));
            }
          }
          ...
          if (train.getStopCount() == 0) {
            arrivalTimeSelector.setText(
                convertDoubleToClockTime(selectedTrain.getDispatchTime() + (60)));
            departureTimeSelector.setText(
                convertDoubleToClockTime(selectedTrain.getDispatchTime() + 120));
          } else {
            if (arrivalTimeSelector.getText().isEmpty()
                || (convertClockTimeToDouble(arrivalTimeSelector.getText())
                    <= train.getStop(train.getStopCount() - 1).getArrivalTime())) {
              arrivalTimeSelector.setText(
                  convertDoubleToClockTime(
                      train.getStop(train.getStopCount() - 1).getDepartureTime() + 300));
            }
            if (departureTimeSelector.getText().isEmpty()
                || (convertClockTimeToDouble(departureTimeSelector.getText())
                    <= train.getStop(train.getStopCount() - 1).getDepartureTime())) {
              departureTimeSelector.setText(
                  convertDoubleToClockTime(
                      convertClockTimeToDouble(arrivalTimeSelector.getText()) + 60));
            }
          }
          train.addStop(
              Integer.parseInt(stationStopSelector.getText()),
              convertClockTimeToDouble(arrivalTimeSelector.getText()),
              convertClockTimeToDouble(departureTimeSelector.getText()));
    ...
    RemoveStop.setOnAction(
        event -> {
          ...
          scheduleEditTable
              .getItems()
              .remove(scheduleEditTable.getItems().get(stopSelector.getValue() - 1));
          stopSelector.getItems().remove(train.getStops().size() - 1);
        });
    saveStopButton.setOnAction(
        event -> {
          ...
          stop.getStop()
              .setArrivalTime((int) convertClockTimeToDouble(arrivalTimeSelector.getText()));
          stop.getStop()
              .setDepartureTime((int) convertClockTimeToDouble(departureTimeSelector.getText()));
  ...
  private void setupScheduleButtons() {
    ...
    RemoveSchedule.setOnAction(
        event -> {
          // System.out.println("removed schedule\n");
          scheduleLibrary.removeScheduleFile(
              scheduleTable
                  .getSelectionModel()
                  .getSelectedItem()
                  .getStringProperty(SCHEDULE_FILE_NAME_PROPERTY)
                  .getValue());
    ...
    checkScheduleButton.setOnAction(
        event -> {
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (mainAnchor.getWidth() > 0) {
                if (Array.getDouble(mainSplit.getDividerPositions(), 0) * mainAnchor.getWidth()
                    < minDividerPosition) {
                  mainSplit.setDividerPosition(0, minDividerPosition / mainAnchor.getWidth());
                } else if ((1 - Array.getDouble(mainSplit.getDividerPositions(), 0))
                        * mainAnchor.getWidth()
                    > maxDividerPosition) {
                  mainSplit.setDividerPosition(0, 1 - (maxDividerPosition / mainAnchor.getWidth()));
                }

                scheduleNameColumn.setMaxWidth(
                    ((newValue.doubleValue() * (1 - mainSplit.getDividerPositions()[0]))
                            - tableWidthAdjustment)
                        * 0.7);
                scheduleDateModColumn.setMaxWidth(
                    ((newValue.doubleValue() * (1 - mainSplit.getDividerPositions()[0]))
                            - tableWidthAdjustment)
                        * 0.3);
                scheduleNameColumn.setMinWidth(
                    ((newValue.doubleValue() * (1 - mainSplit.getDividerPositions()[0]))
                            - tableWidthAdjustment)
                        * 0.7);
                scheduleDateModColumn.setMinWidth(
                    ((newValue.doubleValue() * (1 - mainSplit.getDividerPositions()[0]))
                            - tableWidthAdjustment)
                        * 0.3);
              }
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (minDividerPosition / mainAnchor.getWidth() > newValue.doubleValue()) {
                mainSplit.setDividerPosition(0, minDividerPosition / mainAnchor.getWidth());
              } else if (1 - (maxDividerPosition / mainAnchor.getWidth())
                  < newValue.doubleValue()) {
                mainSplit.setDividerPosition(0, 1 - (maxDividerPosition / mainAnchor.getWidth()));
              }
              scheduleNameColumn.setMaxWidth(
                  ((mainAnchor.getWidth() * (1 - newValue.doubleValue())) - tableWidthAdjustment)
                      * 0.7);
              scheduleDateModColumn.setMaxWidth(
                  ((mainAnchor.getWidth() * (1 - newValue.doubleValue())) - tableWidthAdjustment)
                      * 0.3);
