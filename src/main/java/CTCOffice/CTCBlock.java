...
/**
 * This class represents a block of the track in the Centralized Traffic Control (CTC) system. It
 * contains information about the block such as its ID, occupancy status, presence of light, switch
 * and crossing, their states, speed limit, block length, IDs of converging and diverging blocks,
...
class CTCBlock implements Notifier {
  ...
  /**
   * Constructor for the CTCBlockInfo class. Initializes the block properties and registers the
   * block with the CTCBlockSubjectFactory.
   */
  CTCBlock(BasicBlock block) {
    ...
    if (this.isSwitchCon) {
      ...
      this.divergingBlockOneID =
          BlockIDs.of(
              ((block.nextBlock().primarySwitchDirection() == NORTH)
                  ? block.nextBlock().northDefault().blockNumber()
                  : block.nextBlock().southDefault().blockNumber()),
              blockID.line());
      this.divergingBlockTwoID =
          BlockIDs.of(
              ((block.nextBlock().primarySwitchDirection() == NORTH)
                  ? block.nextBlock().northAlternate().blockNumber()
                  : block.nextBlock().southAlternate().blockNumber()),
              blockID.line());
    }
    subMap.registerSubject(blockID, new CTCBlockSubject(this));
  }

  void setSwitchDivInformation(
      BlockIDs convergingBlockID, BlockIDs divergingBlockOneID, BlockIDs divergingBlockTwoID) {
    ...
    CTCBlockSubjectMap.getInstance()
        .getSubject(this.blockID)
        .updateStringProperty(SWITCH_STATE_STRING_PROPERTY);
  }
  ...
  /**
   * Sets the state of the switch and updates the switch state of the converging and diverging
   * blocks.
   */
  void setSwitchState(boolean GUI, boolean state) {
    ...
    if (!GUI) {
      ...
      if ((convergingBlockID == blockID)
          && ((state
                  != subMap
                      .getSubject(BlockIDs.of(divergingBlockOneID.blockIdNum(), blockID.line()))
                      .getBlockInfo()
                      .getSwitchState())
              || (state
                  != subMap
                      .getSubject(BlockIDs.of(divergingBlockTwoID.blockIdNum(), blockID.line()))
                      .getBlockInfo()
                      .getSwitchState()))) {
        subMap
            .getSubject(BlockIDs.of(divergingBlockOneID.blockIdNum(), blockID.line()))
            .getBlockInfo()
            .setSwitchState(false, state);
        subMap
            .getSubject(BlockIDs.of(divergingBlockTwoID.blockIdNum(), blockID.line()))
            .getBlockInfo()
            .setSwitchState(false, state);
      ...
      if ((divergingBlockOneID.blockIdNum() == blockID.blockIdNum()
              || divergingBlockTwoID.blockIdNum() == blockID.blockIdNum())
          && (subMap
                  .getSubject(BlockIDs.of(convergingBlockID.blockIdNum(), blockID.line()))
                  .getBlockInfo()
                  .getSwitchState()
              != state)) {
      ...
      WaysideSystem.getController(blockID.line(), blockID.blockIdNum())
          .maintenanceSetSwitch(convergingBlockID.blockIdNum(), state);
      logger.info(
          "Wayside called to set switch state to "
              + state
              + " for block "
              + convergingBlockID.blockIdNum()
              + " on line "
              + blockID.line());
    }
  }
  ...
  String getSwitchStateString() {
    ...
    if (isSwitchCon) {
      if (switchState) {
        return (divergingBlockOneID.blockIdNum()
            + "  ( "
            + convergingBlockID.blockIdNum()
            + " == "
            + divergingBlockTwoID.blockIdNum()
            + " )");
      } else {
        return ("( "
            + divergingBlockOneID.blockIdNum()
            + " == "
            + convergingBlockID.blockIdNum()
            + " )  "
            + divergingBlockTwoID.blockIdNum());
  ...
  void setUnderMaintenance(boolean GUI, boolean underMaintenance) {
    if (!GUI) {
      ...
      if (isSwitchCon || isSwitchDiv) {
        logger.info(
            "Wayside called to set maintenance state to "
                + underMaintenance
                + " for switch from block "
                + convergingBlockID.blockIdNum()
                + " on line "
                + blockID.line());
        WaysideSystem.getController(convergingBlockID.line(), convergingBlockID.blockIdNum())
            .CTCChangeBlockMaintenanceState(convergingBlockID.blockIdNum(), underMaintenance);
        WaysideSystem.getController(divergingBlockOneID.line(), divergingBlockOneID.blockIdNum())
            .CTCChangeBlockMaintenanceState(divergingBlockOneID.blockIdNum(), underMaintenance);
        WaysideSystem.getController(divergingBlockTwoID.line(), divergingBlockTwoID.blockIdNum())
            .CTCChangeBlockMaintenanceState(divergingBlockTwoID.blockIdNum(), underMaintenance);
      } else {
        logger.info(
            "Wayside called to set maintenance state to "
                + underMaintenance
                ...
                + blockID.line());
        WaysideSystem.getController(blockID.line(), blockID.blockIdNum())
            .CTCChangeBlockMaintenanceState(blockID.blockIdNum(), underMaintenance);
      }
    }
  }
  ...
  void setSwitchLightState(boolean GUI, boolean switchLightState) {
    ...
    if (!GUI) {
      ...
      notifyChange("switchLightState", switchLightState);
    } else {
      logger.info(
          "Wayside called to set light state to "
              + switchLightState
              + " for block "
              + blockID.blockIdNum()
              + " on line "
              + blockID.line());
      WaysideSystem.getController(blockID.line(), blockID.blockIdNum())
          .maintenanceSetTrafficLight(blockID.blockIdNum(), switchLightState);
    }
  }
  ...
  void setCrossingState(boolean GUI, boolean crossingState) {
    ...
    if (!GUI) {
      ...
      WaysideSystem.getController(blockID.line(), blockID.blockIdNum())
          .maintenanceSetCrossing(blockID.blockIdNum(), crossingState);
      logger.info(
          "Wayside called to set crossing state to "
              + crossingState
              + " for block "
              + blockID.blockIdNum()
              + " on line "
