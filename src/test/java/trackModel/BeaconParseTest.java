  ...
  void testParseBeacons() {
    for (Lines line : Lines.values()) {
      ...
      for (Integer beaconID : beacons.keySet()) {
        ...
        totalSourceBeacons.put(
            beacon.sourceId(), totalSourceBeacons.getOrDefault(beacon.sourceId(), 0) + 1);
        // System.out.print( beaconID + " ");
        if (trackLine.get(beacon.sourceId()).isStation()) {
          // System.out.println(trackLine.get(beacon.sourceId()).stationName().get() + ": " +
          // beacon.sourceId() + " -> " + beacon.endId());
        } else {
      ...
      for (Integer sourceID : totalSourceBeacons.keySet()) {
        if (trackLine.get(sourceID).isSwitch() && totalSourceBeacons.get(sourceID) != 3) {
          // System.out.println("Switch " + sourceID + " has "+ totalSourceBeacons.get(sourceID)  +"
          // beacons");
        } else if (trackLine.get(sourceID).isStation() && totalSourceBeacons.get(sourceID) != 2) {
          // System.out.println("Station " + sourceID + " has "+ totalSourceBeacons.get(sourceID) +"
          // beacons");
        }
      }
    }
